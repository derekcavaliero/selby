(function($, window, document, undefined) {
    ...
    Selby.prototype = {
        ...
        processStep: function(stepKey) {
            if (stepKey) {
                ...
                if (step.hasOwnProperty("prompt")) {
                    ...
                    if (step.fields) userData.fields = this.renderFields(step.fields, stepKey);
                    var userPrompt = $(tpl.user(userData)).appendTo(this.element);
                }
                if (this.options.onStepReady.constructor === Function && step.prompt) this.options.onStepReady(userPrompt);
            }
        ...
        renderFields: function(fieldKeys, stepKey) {
            var parent = this, output = "", fieldCount = fieldKeys.length, outputButton = false, needsButtonTypes = [ "text", "tel", "email", "number", "textarea", "checkbox" ], fieldDataAttr = ' data-proceed-on-change="' + (fieldCount === 1 ? "true" : "false") + '"';
            fieldKeys.forEach(function(key) {
                ...
                if (field.label) output += '<label class="bubble__input-label" for="' + key + 'Field">' + field.label + "</label>";
                switch (field.type) {
                  case "text":
                  case "email":
                  case "number":
                ...
                output += "</div>";
                if (fieldCount > 1 || needsButtonTypes.indexOf(field.type) !== -1) outputButton = true;
            });
            if (outputButton) output += '<button type="submit" class="bubble__button">Continue</button>';
            return output;
        },
        ...
        processData: function() {
            ...
            delete payload.step;
            if (this.options.dataFilter.constructor === Function) payload = this.options.dataFilter(payload);
            $.ajax({
                url: url,
                method: "POST",
                type: "json",
        ...
        transformInput: function(inputObj) {
            for (var field in inputObj) {
                ...
                if ("name" == field) {
                    ...
                    if (nameParts[1]) inputObj.lastname = this.ucFirst(nameParts[1]);
